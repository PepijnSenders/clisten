...
/// Save a SoundCloud OAuth token to the config file.
pub fn save_soundcloud_token(token: &str) -> anyhow::Result<()> {
    let mut config = Config::load()?;
    config.soundcloud.oauth_token = token.to_string();
    config.save()
}
```
