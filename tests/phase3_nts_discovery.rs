...
fn open_temp_db() -> Database {
    ...
    Database::open_at(&path).expect("open db")
}
...
fn test_database_add_favorite() {
    ...
    assert!(db.is_favorite("nts:episode:test-show:test-show-ep1").expect("is_favorite"));
}
...
fn test_database_remove_favorite() {
    ...
    db.add_favorite(&item).expect("add_favorite");
    assert!(db.is_favorite("nts:episode:test-show:test-show-ep1").expect("is_favorite"));
    db.remove_favorite("nts:episode:test-show:test-show-ep1").expect("remove_favorite");
    assert!(!db.is_favorite("nts:episode:test-show:test-show-ep1").expect("is_favorite"));
}
...
fn test_database_list_favorites_by_source() {
    ...
    let direct_only = db.list_favorites(Some("direct"), 100, 0).expect("list direct");
...
fn test_database_clear_history() {
    let db = open_temp_db();
    db.add_to_history(&make_episode("Show A", "ep-a")).expect("add");
    db.add_to_history(&make_episode("Show B", "ep-b")).expect("add");
...
fn test_favorite_record_to_discovery_item() {
    ...
    assert!(matches!(discovery, clisten::api::models::DiscoveryItem::NtsEpisode { .. }));
}
...
fn test_number_keys_send_switch_sub_tab() {
    ...
    for (digit, expected_idx) in [('1', 0), ('2', 1), ('3', 2)] {
        ...
        assert_eq!(idx, expected_idx, "key '{}' should map to sub-tab {}", digit, expected_idx);
    }
}
...
fn test_number_keys_ignored_in_search() {
    ...
    assert_eq!(bar.input, "123", "digits should be typed into search input");
}
...
fn test_discovery_list_append_items() {
    ...
    assert_eq!(list.all_items.len(), 1);
...
fn test_discovery_list_append_items_with_filter() {
    ...
    assert_eq!(list.all_items.len(), 3);
    assert_eq!(list.items.len(), 2); // both jazz items visible
}
